...
export class Bookmarks {
    async getFromStartPage(): Promise<[chrome.bookmarks.BookmarkTreeNode[], chrome.bookmarks.BookmarkTreeNode[]]> {
        const bookmarks = await chrome.runtime.sendMessage(GetBookmarksMessage);

        const flattened = bookmarks.map(
            (v: chrome.bookmarks.BookmarkTreeNode) => flatten(v)
        ).flat();
