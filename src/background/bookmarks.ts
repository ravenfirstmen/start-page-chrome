...
export const handleGetBookmarks = async (sendResponse: GetBookmarksMessageResponse) => {
    console.log('Received get bookmarks request');
    sendResponse(cache);
    return true;
}

export const handleOnInstall = async (details: chrome.runtime.InstalledDetails) => {
...
const loadBookmarks = async () => {
    ...
    if (anchor.length == 1) { // found the anchor
        const startpageBookmarks = await chrome.bookmarks.getSubTree(anchor[0].id);
        const startpageFolders = !startpageBookmarks.length ? [] : startpageBookmarks[0].children ?? [];
