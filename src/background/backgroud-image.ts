...
const MAX_IMAGES = 20;
const FILTERS = 'image_type=photo&category=backgrounds&min_width=1280&safesearch=true&editors_choice=true&order=latest&page=1';
...
export async function cacheBackgroundImage(): Promise<string> {
    ...
    const currentInfo = await chrome.storage.local.get([LAST_IMAGE_STORAGE_KEY, LAST_IMAGE_ROTATION_TIME_STORAGE_KEY]);
    ...
    if (!backgroundImage) {
        ...
        if (backgroundImage) {
            await chrome.storage.local.set({ lastImage: backgroundImage, date: Date.now() });
...
async function setRotateImageAlarm(periodInMinutes: number) {
    ...
    await chrome.alarms.create(ROTATE_IMAGE_ALARM, { periodInMinutes: periodInMinutes });
}

const refreshImage = async (alarm: chrome.alarms.Alarm) => {
    if (alarm.name == ROTATE_IMAGE_ALARM) {
        await chrome.storage.local.remove([LAST_IMAGE_STORAGE_KEY, LAST_IMAGE_ROTATION_TIME_STORAGE_KEY]);
        const imageUrl = await cacheBackgroundImage();
        if(imageUrl) {
            await chrome.runtime.sendMessage(chrome.runtime.id, new SetbackgroundImageMessage(imageUrl));
        }
    }
};
